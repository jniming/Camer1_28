...
public class PlayDeviceActivity extends BaseActivity implements PlayListener,
    ...
    private int count = 0;
    private long secClick, firClick;   //点击的时间

    private boolean check_full = false;
...
//            }
    ...
    private void initView() {
        ...
        vidio_btn.setOnClickListener(this);
        sound_btn.setOnClickListener(this);
        hd_btn.setOnClickListener(this);
        glSurfaceView1.setOnTouchListener(new View.OnTouchListener() {
            @Override
...
//        handler.sendEmptyMessage(0);
    ...
    private void fullsrcee() {
        if (!flg) {
            flg = true;
            full_img.setImageResource(R.drawable.full_into_img);
            if (getRequestedOrientation() != ActivityInfo.SCREEN_ORIENTATION_LANDSCAPE) {
                setRequestedOrientation(ActivityInfo.SCREEN_ORIENTATION_LANDSCAPE);
            }
        } else {
            flg = false;
            full_img.setImageResource(R.drawable.full_out_img);
